...
export function costOf(task: PlanTask, settings: PlanSettings): { hours: number; isUnestimated: boolean } {
  ...
  if (positive(task.estimatedHours) > 0) {
    return { hours: Math.max(0.25, positive(task.estimatedHours) - positive(task.loggedHours)), isUnestimated: false };
  }
...
export function buildPlan(
  ...
  const updateState = (day: DayPlan) => {
    day.ratio = day.capacityHours === 0 ? 0 : day.committedHours / day.capacityHours;
    day.state = day.capacityHours === 0 ? 'closed' : day.ratio > 1 ? 'over' : day.ratio >= 0.9 ? 'tight' : 'ok';
  };
  const add = (day: DayPlan, item: PlanItem, consumes = true) => {
    day.items.push(item);
    if (consumes) day.committedHours += item.costHours;
    updateState(day);
  };
  const placementState = (task: PlanTask, day: string, originalDay: string): PlanState =>
    (task.dueDate && (task.dueDate < today || day > task.dueDate)) ? 'late' : day === originalDay ? 'scheduled' : 'deferred';
  const unplannable = (task: PlanTask, cost: number, unestimated: boolean) => {
    add(days[0], { taskId: task.id, costHours: cost, totalCostHours: cost, state: 'unplannable', originalDay: today, dueDate: task.dueDate, isUnestimated: unestimated }, false);
  };

  const pinned = ordered.filter((task) => task.pinnedDay);
  for (const task of pinned) {
    const { hours, isUnestimated } = costOf(task, settings);
    const targetIndex = Math.max(0, days.findIndex((day) => day.day === task.pinnedDay));
    const target = days[targetIndex];
    add(target, { taskId: task.id, costHours: hours, totalCostHours: hours, state: 'pinned', dueDate: task.dueDate, isUnestimated: isUnestimated });
  }

  for (const task of ordered.filter((candidate) => !candidate.pinnedDay)) {
    const { hours, isUnestimated } = costOf(task, settings);
    if (hours === 0) {
      const target = days.find((day) => day.capacityHours > 0) ?? days[0];
      add(target, { taskId: task.id, costHours: 0, totalCostHours: 0, state: placementState(task, target.day, today), originalDay: target.day === today ? undefined : today, dueDate: task.dueDate, isUnestimated });
      continue;
    }
    ...
    if (hours > maxDailyCapacity) {
      let remaining = hours;
      const placements: { day: DayPlan; cost: number }[] = [];
      for (const day of days) {
        if (remaining <= 0) break;
        const available = Math.max(0, day.capacityHours - day.committedHours);
        if (available <= 0) continue;
        const segment = Math.min(available, remaining);
        placements.push({ day, cost: segment });
        remaining -= segment;
      }
      if (remaining > 0) { unplannable(task, hours, isUnestimated); continue; }
      placements.forEach(({ day, cost }, index) => add(day, {
        taskId: task.id, costHours: cost, totalCostHours: hours, state: 'split', segmentIndex: index + 1,
        segmentCount: placements.length, originalDay: day.day === today ? undefined : today, dueDate: task.dueDate, isUnestimated,
      }));
      continue;
    }
